...
import { espetaculos } from "../data/portfolio";
      ...
      <div className="relative max-w-5xl mx-auto px-6">
        ...
        <div className="space-y-1 reveal-stagger">
          {espetaculos.map((item, i) => {
            ...
            const contentId = `espetaculo-content-${i}`;
            const buttonId = `espetaculo-button-${i}`;
            ...
            return (
              ...
              >
                <button
                  ...
                  className={`relative w-full text-left p-5 md:p-6 rounded-sm border transition-all duration-500 focus-visible:ring-2 focus-visible:ring-gold outline-none ${isExpanded
                ...
                >
                  <div className="flex flex-col md:flex-row md:items-center gap-2 md:gap-8">
                    ...
                    {/* Name */}
                    <h3 className={`font-display text-2xl md:text-3xl tracking-[0.04em] flex-1 transition-colors duration-500 ${isExpanded ? "text-gold" : "text-off-white group-hover:text-gold/80"
                      }`}>
                      {item.name}
                    </h3>
                    ...
                    {/* Toggle indicator */}
                    <span className={`font-mono text-[10px] text-warm-gray/40 transition-transform duration-300 hidden md:block ${isExpanded ? "rotate-45" : ""
                      }`}>
                      +
                    </span>
                  </div>
                  ...
                  <div
                    ...
                    className={`overflow-hidden transition-all duration-500 ease-out ${isExpanded ? "max-h-48 opacity-100 mt-4" : "max-h-0 opacity-0"
                      }`}
                  >
                  ...
                  {/* Bottom line accent */}
                  <div aria-hidden="true" className={`absolute bottom-0 left-0 h-[1px] bg-gradient-to-r from-gold/40 to-transparent transition-all duration-700 ${isExpanded ? "w-full" : "w-0 group-hover:w-1/3"
                    }`} />
                </button>
              </div>
            );
        ...
        <div className="reveal mt-16 pt-8 border-t border-white/[0.05] flex items-center justify-between">
          ...
          <p className="font-mono text-[10px] tracking-[0.2em] uppercase text-warm-gray/30">
            {espetaculos[espetaculos.length - 1].year} – {espetaculos[0].year}
          </p>
        </div>
      </div>
    </section>
