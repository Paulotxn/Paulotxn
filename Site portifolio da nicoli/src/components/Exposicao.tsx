...
import { references, technicalDetails } from "../data/portfolio";
              ...
              <div className="flex flex-wrap gap-2">
                {references.map((ref, i) => (
                  <button
                    ...
                    onFocus={() => setHoveredRef(ref.name)}
                    onBlur={() => setHoveredRef(null)}
                    className={`relative px-4 py-2 font-serif text-sm rounded-sm border transition-all duration-400 outline-none focus-visible:ring-1 focus-visible:ring-gold text-left ${hoveredRef === ref.name
                      ? "border-gold/40 text-gold bg-gold/[0.04]"
                      : "border-white/[0.06] text-warm-gray/70 hover:border-white/[0.12] hover:text-off-white"
                      }`}
                  >
                    {ref.name}
                    <span className={`absolute -top-1 -right-1 font-mono text-[7px] tracking-wider uppercase px-1.5 py-0.5 rounded-full bg-dark-bg border border-white/10 text-warm-gray/40 transition-opacity duration-300 ${hoveredRef === ref.name ? "opacity-100" : "opacity-0"
                      }`}>
                      {ref.category}
                    </span>
                  </button>
        ...
        <div className="reveal mt-20">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            {references.map((ref, i) => {
              ...
              const currentImage = getImage(imgKey);
